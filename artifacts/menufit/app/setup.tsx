...
export default function SetupScreen() {
  ...
  const [shoppingTrips, setShoppingTrips] = useState('1');
  ...
  const finish = () => {
    completeSetup({ people: Math.max(1, Number(people) || 2), days: Math.min(30, Math.max(1, Number(days) || 5)), mealsPerDay: Math.min(4, Math.max(2, Number(meals) || 4)), shoppingTripsPerWeek: Math.min(7, Math.max(1, Number(shoppingTrips) || 1)), goal, cookTime, diet, allergies: allergies.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean), favorites: favorites.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean), pantry: pantry.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean) });
    router.replace('/home');
  };
  return <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top + 12 }]}>
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      {step === 0 ? <View><Text style={[styles.kicker, { color: colors.primary }]}>EMPECEMOS</Text><Text style={[styles.title, { color: colors.foreground }]}>Cuéntanos para quién cocinas.</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Crearemos un plan que encaje de verdad con tu semana.</Text><Field label="Personas en casa" value={people} onChangeText={setPeople} placeholder="2" /><Field label="Días a planificar (1–30)" value={days} onChangeText={setDays} placeholder="5" /><Text style={[styles.hint, { color: colors.mutedForeground }]}>Puedes crear desde un plan diario hasta un calendario completo de un mes.</Text><Text style={[styles.label, { color: colors.mutedForeground }]}>Comidas al día</Text><View style={styles.pills}>{['3', '4'].map((value) => <Pill key={value} label={`${value} comidas`} selected={meals === value} onPress={() => setMeals(value)} />)}</View></View> : null}
      ...
      {step === 2 ? <View><Text style={[styles.kicker, { color: colors.primary }]}>A TU MEDIDA</Text><Text style={[styles.title, { color: colors.foreground }]}>Una última vuelta.</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Puedes cambiarlo todo después desde tu perfil.</Text><Text style={[styles.label, { color: colors.mutedForeground }]}>Tiempo máximo para cocinar</Text><View style={styles.pills}>{['5-10 minutos', '10-20 minutos', '20-30 minutos', 'Más de 45 minutos'].map((value) => <Pill key={value} label={value} selected={cookTime === value} onPress={() => setCookTime(value)} />)}</View><Text style={[styles.label, { color: colors.mutedForeground }]}>Tipo de alimentación</Text><View style={styles.pills}>{['Sin restricciones', 'Vegetariana', 'Sin pescado', 'Vegana'].map((value) => <Pill key={value} label={value} selected={diet === value} onPress={() => setDiet(value)} />)}</View><Text style={[styles.label, { color: colors.mutedForeground }]}>¿Cuántas veces compras por semana?</Text><View style={styles.pills}>{['1', '2', '3', '7'].map((value) => <Pill key={value} label={value === '7' ? 'Cada día' : `${value} ${value === '1' ? 'vez' : 'veces'}`} selected={shoppingTrips === value} onPress={() => setShoppingTrips(value)} />)}</View><Text style={[styles.hint, { color: colors.mutedForeground }]}>La lista de compra marcará lo imprescindible hasta tu próxima visita.</Text><Field label="Alergias o intolerancias" value={allergies} onChangeText={setAllergies} placeholder="Ej. frutos secos, gluten" /><Field label="Alimentos favoritos" value={favorites} onChangeText={setFavorites} placeholder="Ej. pollo, aguacate" /><Field label="Lo que ya tienes en casa" value={pantry} onChangeText={setPantry} placeholder="Ej. arroz, huevos" multiline /></View> : null}
    </ScrollView>
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: 12, lineHeight: 18, marginTop: -20, marginBottom: 22, maxWidth: 320 },
