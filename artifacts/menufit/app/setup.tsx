...
export default function SetupScreen() {
  ...
  const finish = () => {
    completeSetup({ people: Math.max(1, Number(people) || 2), days: Math.min(30, Math.max(1, Number(days) || 5)), mealsPerDay: Math.min(4, Math.max(2, Number(meals) || 4)), shoppingDays: Math.min(30, Math.max(1, Number(shoppingDays) || 7)), goal, cookTime, diet, allergies: allergies.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean), favorites: favorites.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean), pantry: pantry.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean) });
    router.replace('/home');
  };
