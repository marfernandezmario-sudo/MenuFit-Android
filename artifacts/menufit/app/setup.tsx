...
export default function SetupScreen() {
  ...
  const finish = () => {
    completeSetup({ people: Math.max(1, Number(people) || 2), days: Math.min(30, Math.max(1, Number(days) || 5)), mealsPerDay: Math.min(4, Math.max(2, Number(meals) || 4)), goal, cookTime, diet, allergies: allergies.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean), favorites: favorites.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean), pantry: pantry.split(',').map((item) => item.trim().toLowerCase()).filter(Boolean) });
    router.replace('/home');
  };
  return <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top + 12 }]}>
    ...
    <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {step === 2 ? <View><Text style={[styles.kicker, { color: colors.primary }]}>A TU MEDIDA</Text><Text style={[styles.title, { color: colors.foreground }]}>Una última vuelta.</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>Puedes cambiarlo todo después desde tu perfil y desde Compra.</Text><Text style={[styles.label, { color: colors.mutedForeground }]}>Tiempo máximo para cocinar</Text><View style={styles.pills}>{['5-10 minutos', '10-20 minutos', '20-30 minutos', 'Más de 45 minutos'].map((value) => <Pill key={value} label={value} selected={cookTime === value} onPress={() => setCookTime(value)} />)}</View><Text style={[styles.label, { color: colors.mutedForeground }]}>Tipo de alimentación</Text><View style={styles.pills}>{['Sin restricciones', 'Vegetariana', 'Sin pescado', 'Vegana'].map((value) => <Pill key={value} label={value} selected={diet === value} onPress={() => setDiet(value)} />)}</View><Field label="Alergias o intolerancias" value={allergies} onChangeText={setAllergies} placeholder="Ej. frutos secos, gluten" /><Field label="Alimentos favoritos" value={favorites} onChangeText={setFavorites} placeholder="Ej. pollo, aguacate" /><Field label="Lo que ya tienes en casa" value={pantry} onChangeText={setPantry} placeholder="Ej. arroz, huevos" multiline /></View> : null}
    </ScrollView>
