...
export default function MenuScreen() {
  ...
  const visibleDays = useMemo(() => dayNames.slice(0, preferences.days), [preferences.days]);
  return <ScrollView style={{ backgroundColor: colors.background }} contentContainerStyle={[styles.content, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 100 }]} showsVerticalScrollIndicator={false}>
    <View style={styles.header}><View><Text style={[styles.eyebrow, { color: colors.primary }]}>PLAN SEMANAL</Text><Text style={[styles.title, { color: colors.foreground }]}>Tu menú</Text><Text style={[styles.subtitle, { color: colors.mutedForeground }]}>{preferences.days} días · {preferences.people} {preferences.people === 1 ? 'persona' : 'personas'}</Text></View><IconButton icon="refresh-cw" onPress={() => generateMenu()} label="Regenerar menú completo" /></View>
    <View style={[styles.tip, { backgroundColor: colors.secondary }]}><Feather name="sliders" size={16} color={colors.secondaryForeground} /><Text style={[styles.tipText, { color: colors.secondaryForeground }]}>Hemos priorizado aprovechar ingredientes y mantener variedad.</Text></View>
