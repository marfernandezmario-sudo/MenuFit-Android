...
export default function ProfileScreen() {
  ...
  return <ScrollView style={{ backgroundColor: colors.background }} contentContainerStyle={[styles.content, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 100 }]} showsVerticalScrollIndicator={false}>
    ...
    <Text style={[styles.label, { color: colors.mutedForeground }]}>Días que quieres cubrir con esta compra</Text><View style={styles.pills}>{[1, 3, 7, 14, 30].map((days) => <Pill key={days} label={`${days} ${days === 1 ? 'día' : 'días'}`} selected={preferences.shoppingDays === days} onPress={() => updatePreferences({ shoppingDays: days })} />)}</View>
