...
export default function ProfileScreen() {
  ...
  const { preferences, updatePreferences, generateMenu, clearCurrentMenu, addExclusion, removeExclusion, history, restoreHistory, deleteHistory } = useMenuFit();
  ...
  const confirmClearMenu = () => Alert.alert('Empezar de cero', 'Se borrará el menú actual y los productos generados en la lista de compra. Tus favoritos, historial y productos personalizados se conservarán.', [{ text: 'Cancelar', style: 'cancel' }, { text: 'Borrar menú', style: 'destructive', onPress: clearCurrentMenu }]);
  return <ScrollView style={{ backgroundColor: colors.background }} contentContainerStyle={[styles.content, { paddingTop: insets.top + 12, paddingBottom: insets.bottom + 100 }]} showsVerticalScrollIndicator={false}>
    ...
    <Field label="Número de personas" value={String(preferences.people)} onChangeText={(value) => { const digits = value.replace(/\D/g, ''); if (digits) updatePreferences({ people: Math.min(12, Math.max(1, Number(digits))) }); }} placeholder="2" keyboardType="number-pad" />
    ...
    <Text style={[styles.label, { color: colors.mutedForeground }]}>Comidas al día</Text><View style={styles.pills}>{[2, 3, 4].map((meals) => <Pill key={meals} label={`${meals} comidas`} selected={preferences.mealsPerDay === meals} onPress={() => updatePreferences({ mealsPerDay: meals })} />)}</View>
    <Text style={[styles.label, { color: colors.mutedForeground }]}>Tipo de alimentación</Text><View style={styles.pills}>{['Sin restricciones', 'Vegetariana', 'Sin pescado', 'Vegana'].map((diet) => <Pill key={diet} label={diet} selected={preferences.diet === diet} onPress={() => updatePreferences({ diet })} />)}</View>
    ...
    <SectionTitle eyebrow="PLAN ACTUAL" title="Empezar de cero" />
    <Text style={[styles.resetCopy, { color: colors.mutedForeground }]}>Borra el menú y vuelve a generarlo con tus preferencias actuales.</Text>
    <PrimaryButton label="Borrar menú actual" icon="trash-2" secondary onPress={confirmClearMenu} />
...
const styles = StyleSheet.create({
  ...
  resetCopy: { fontSize: 13, lineHeight: 19, marginBottom: 12 },
